...
import com.gk.htc.sendMT.VNPTNET.Send2VNPTNET;
 ...
 * @author TUANPLA
 */
public class VNPTNET_Task extends _AbstractThreadSend2Provider {

    private static final Logger logger = Logger.getLogger(VNPTNET_Task.class);
    private static final WorkQueue WORKQUEUE_VINAPHONE = new WorkQueue("WORKQUEUE_VINAPHONE", 5);
    private static final String QUEUE_NAME = "Q-->SEND_TO_VINAPHONE";

    public VNPTNET_Task() {
        super(QUEUE_NAME);
        TPS = WORKQUEUE_VINAPHONE.getMaxPoolSize();
        this.setName("VNPTNET_Task [" + DateProc.createTimestamp() + "]");
        MonitorWorker.addDemonName(this.getName());
        MonitorWorker.addWorkQueue(WORKQUEUE_VINAPHONE);
    }
        ...
        while (AppStart.isRuning && !stop) {
            try {
                ...
                if (!oneQueueBr.isShutDown()) {
                    WORKQUEUE_VINAPHONE.execute(doSendBrand_runnable(oneQueueBr));
                }
    ...
    protected Runnable doSendBrand_runnable(final SmsBrandQueue oneQueue) {
        return new Runnable() {
            ...
            public void run() {
                ...
                try {
                    // Submit
                    String[] tmp = Send2VNPTNET.sendOneQueue(oneQueue);
                    long delay = working.done();
                    oneQueue.setProcessTime(delay);
                    if (tmp[0].equals(Send2VNPTNET.STATUS.SUCCESS.val + "")) {
                        oneQueue.setResult(SUCCESS_INT);
                        oneQueue.setErrorInfo(SUCCESS_MSG);
                    } else if (tmp[0].equals(Send2VNPTNET.STATUS.EXCEPTION.val + "")
                            || tmp[0].equals(Send2VNPTNET.STATUS.PARSE_RESULT_ERROR.val + "")) {
                        // Qua TPS thi Log Lai de gui qua 3 lan thi log Submit thanh loi
                        int retry = oneQueue.getRetry();
                        if (retry < 3) {
                            oneQueue.setRetry();
                            oneQueue.setCacheFrom(" Send2_VINAPHONE_Task.UNKNOW_ERROR");
                    ...
                    oneQueue.setCacheFrom(" Send2_VINAPHONE_Task:" + e.getMessage());
