 ...
 * @author NGOC LONG
 */
public class NewProcessV2 {

    static final Logger logger = Logger.getLogger(NewProcessV2.class);
    ...
    private final RequestMessageV2 reqMsg;
    public static final int _DEV = 1;

    public NewProcessV2(RequestMessageV2 reqMsg) {
        this.reqMsg = reqMsg;
    }
    ...
    public ResponseMessage process() {
        ...
        System.out.println("operByPhone:" + operByPhone);
...
//            }
            ...
            if (acc.getStatus() != Account.STATUS.ACTIVE.val) {
                ...
                ///////============ 
                String optStr = brand.getOptionTelco();
                if (Tool.checkNull(optStr)) {
                    oneReqQueue.setResult(CODE.DEV_INVALID.val);
                    oneReqQueue.setErrorInfo(CODE.DEV_INVALID.mess);
                    _resp.setCode(CODE.DEV_INVALID.val);
                    _resp.setMessage("optStr null");
                    return _resp;
                }
                OptionTelco opt = OptionTelco.json2Objec(optStr);
                System.out.println("optStr:" + optStr);

                OptionCheckDuplicate opt_checkDuplicate = opt.getCheckDuplicate();

                boolean checkDuplicateTelco = SMSUtils.checkDuplicateTelco(operByPhone, opt_checkDuplicate);

                // CHECK TIN NHAN TRUNG
                // neu getCheckDuplicate == true || checkDuplicateTelco = true
//                if (reqMsg.getCheckDuplicate() || checkDuplicateTelco) {
                if (checkDuplicateTelco) {
                    System.out.println("yeu cau check duplicate mess");
                    ...
                    System.out.println("bo qua check trung tin nhan");
                }
                ...
                if (_DEV == 1) {
                    ...
                    oneReqQueue.setErrorInfo(CODE.DEV_INVALID.mess);
                    _resp.setCode(CODE.DEV_INVALID.val);
                    _resp.setMessage(CODE.DEV_INVALID.mess);
                    return _resp;
                }
                //--
