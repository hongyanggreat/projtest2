...
import com.gk.htc.sendMT.VIHAT.Send2ViHat;
 ...
 * @author TUANPLA
 */
public class VIHAT_Task extends _AbstractThreadSend2Provider {

    private static final Logger logger = Logger.getLogger(VIHAT_Task.class);
    private static final WorkQueue WORKQUEUE_VIHAT = new WorkQueue("WORKQUEUE_VIHAT", 10);
    private static final String QUEUE_NAME = "Q-->SEND_TO_VIHAT";

    public VIHAT_Task() {
        super(QUEUE_NAME);
        TPS = WORKQUEUE_VIHAT.getMaxPoolSize();
        this.setName("VIHAT_Task [" + DateProc.createTimestamp() + "]");
        MonitorWorker.addDemonName(this.getName());
        MonitorWorker.addWorkQueue(WORKQUEUE_VIHAT);
    }
        ...
        while (AppStart.isRuning && !stop) {
            try {
                ...
                if (!oneQueueBr.isShutDown()) {
                    WORKQUEUE_VIHAT.execute(doSendBrand_runnable(oneQueueBr));
                }
            ...
            public void run() {
                ...
                try {
                    ...
                    if (tmp[0].equals(Send2ViHat.STATUS.SUCCESS.val + "")) {
                        ...
                        oneQueue.setErrorInfo(Send2ViHat.STATUS.getmessage(Tool.getInt(tmp[0])));
                    }
                    logData(oneQueue);
                } catch (Exception e) {
                    logger.error(Tool.getLogMessage(e));
                    ...
                    oneQueue.setCacheFrom(" Send2_VIHAT_Task:" + e.getMessage());
