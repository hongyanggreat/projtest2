 ...
 * @author TUANPLA
 */
public class VNPTMedia_Task extends _AbstractThreadSend2Provider {

    final Logger logger = Logger.getLogger(VNPTMedia_Task.class);
    private static final String USER = "htc-sms2";
    private static final String PASS = "b50f5e5480e4d2e953ff924e38d30be3";
    private static final WorkQueue WORKQUEUE_VNPT_MEDIA_MAIN = new WorkQueue("WORKQUEUE_VNPT_MEDIA_MAIN", 10);
    private static final String QUEUE_NAME = "Q-->SEND_TO_VNPT_MEDIA_MAIN";

    public VNPTMedia_Task() {
        super(QUEUE_NAME);
        TPS = WORKQUEUE_VNPT_MEDIA_MAIN.getMaxPoolSize();
        this.setName("VNPTMedia_Task [" + DateProc.createTimestamp() + "]");
        MonitorWorker.addDemonName(this.getName());
        MonitorWorker.addWorkQueue(WORKQUEUE_VNPT_MEDIA_MAIN);
    }
        ...
        while (AppStart.isRuning && !stop) {
            try {
                ...
                if (!oneQueueBr.isShutDown()) {
                    WORKQUEUE_VNPT_MEDIA_MAIN.execute(doSendBrand_runnable(oneQueueBr));
                }
    ...
    protected Runnable doSendBrand_runnable(final SmsBrandQueue oneQueue) {
        return new Runnable() {
            ...
            public void run() {
                ...
                try {
                    // Submit
                    int repResult = doSendMT(oneQueue.getPhone(), oneQueue.getLabel(), oneQueue.getMessage());
                    // Gui Xong
    ...
    private static int doSendMT(String msisdn, String brandname, String msgbody) {
...
//        String msg = Tool.replace(msgbody, "*", "x");
    ...
    private static int uploadSMS(java.lang.String username, java.lang.String password, java.lang.String serviceId, java.lang.String userId, java.lang.String contentType, java.lang.String serviceKind, java.lang.String infor) {
        com.gk.htc.sendMT.VNPTMedia.BrandNameWSService service = new com.gk.htc.sendMT.VNPTMedia.BrandNameWSService();
        com.gk.htc.sendMT.VNPTMedia.BrandNameWS port = service.getBrandNameWSPort();
        return port.uploadSMS(username, password, serviceId, userId, contentType, serviceKind, infor);
    }
}
