...
public class NewProcess {
    ...
    static final Logger logger = Logger.getLogger(NewProcess.class);
    ...
    public NewProcess(RequestMessage reqMsg) {
        this.reqMsg = reqMsg;
    }
...
//                return _resp;
//            }
            //check duplicate mess
            BrandLabel brand = null;
            if (acc.getStatus() == Account.STATUS.ACTIVE.val) {
                brand = BrandLabel.findFromCache(acc.getUserName(), reqMsg.getBrandName());

                boolean checkDuplicateTelco = true;
                String optStr = brand.getOptionTelco();
                OptionTelco opt = null;
                OptionCheckDuplicate opt_checkDuplicate = null;
                if (!Tool.checkNull(optStr)) {
                    System.out.println("optStr:" + optStr);
                    opt = OptionTelco.json2Objec(optStr);
                }
                if (!Tool.checkNull(opt)) {
                    System.out.println("opt:" + opt);
                    opt_checkDuplicate = opt.getCheckDuplicate();
                }
                if (!Tool.checkNull(opt_checkDuplicate)) {
                    System.out.println("opt_checkDuplicate:" + opt_checkDuplicate);
                    checkDuplicateTelco = SMSUtils.checkDuplicateTelco(operByPhone, opt_checkDuplicate);
                }
                System.out.println("checkDuplicateTelco:" + checkDuplicateTelco);
