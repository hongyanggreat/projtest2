...
import com.gk.htc.sendMT.MFS.MBSVerify.SendVerify2MBSViaMFS;
 ...
 * @author DUONGNH
 */
public class MBS_Verify_VIA_MFS_Task extends _AbstractThreadSend2Provider {

    private static final Logger logger = Logger.getLogger(MBS_Verify_VIA_MFS_Task.class);
    ...
    public MBS_Verify_VIA_MFS_Task() {
        super(QUEUE_NAME);
    ...
    protected Runnable doSendBrand_runnable(final SmsBrandQueue oneQueue) {
        return new Runnable() {
            ...
            public void run() {
                ...
                try {
                    // Submit
                    String[] tmp = SendVerify2MBSViaMFS.sendOneQueue(oneQueue);
                    long delay = working.done();
                    oneQueue.setProcessTime(delay);
                    if (tmp[0].equals(SendVerify2MBSViaMFS.STATUS.SUCCESS.val + "")) {
                        oneQueue.setResult(SUCCESS_INT);
                        oneQueue.setErrorInfo(SUCCESS_MSG);
                    } else if (tmp[0].equals(SendVerify2MBSViaMFS.STATUS.UNKNOW_ERROR.val + "")
                            || tmp[0].equals(SendVerify2MBSViaMFS.STATUS.ERR_PASER.val + "")) {
                        // Qua TPS thi Log Lai de gui qua 3 lan thi log Submit thanh loi
                        int retry = oneQueue.getRetry();
                        if (retry < 3) {
                            oneQueue.setRetry();
                            oneQueue.setCacheFrom("SEND_TO_MBSVerifyViaMFS_Task.UNKNOW_ERROR");
                    ...
                    oneQueue.setCacheFrom(" SEND_TO_MBSVerifyViaMFS_Task:" + e.getMessage());
