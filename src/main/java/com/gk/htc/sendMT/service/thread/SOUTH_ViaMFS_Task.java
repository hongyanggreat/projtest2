...
import com.gk.htc.sendMT.SOUTH.MFS.MFS2South;
 ...
 * @author TUANPLA
 */
public class SOUTH_ViaMFS_Task extends _AbstractThreadSend2Provider {

    private static final Logger logger = Logger.getLogger(SOUTH_ViaMFS_Task.class);
    private static final WorkQueue WORKQUEUE_SOUTH_MFS = new WorkQueue("WORKQUEUE_SOUTH_MFS", 100);
    private static final String QUEUE_NAME = "Q-->SEND_TO_SOUTH_MFS";

    public SOUTH_ViaMFS_Task() {
        super(QUEUE_NAME);
        TPS = WORKQUEUE_SOUTH_MFS.getMaxPoolSize();
        this.setName("SOUTH_MFS_Task [" + DateProc.createTimestamp() + "]");
        MonitorWorker.addDemonName(this.getName());
        MonitorWorker.addWorkQueue(WORKQUEUE_SOUTH_MFS);
    }
        ...
        while (AppStart.isRuning && !stop) {
            try {
                ...
                if (!oneQueueBr.isShutDown()) {
                    WORKQUEUE_SOUTH_MFS.execute(doSendBrand_runnable(oneQueueBr));
                }
    ...
    protected Runnable doSendBrand_runnable(final SmsBrandQueue oneQueue) {
        return new Runnable() {
            ...
            public void run() {
                ...
                try {
                    // Submit
                    SouthResponse resp = MFS2South.sendOneQueue(oneQueue);
                    ...
                    oneQueue.setCacheFrom(" Send2_SOUTH_MFS_Task:" + e.getMessage());
