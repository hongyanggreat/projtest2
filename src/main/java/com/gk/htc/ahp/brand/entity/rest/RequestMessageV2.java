 ...
 * @author Company
 */
public class RequestMessageV2 {

    static final Logger logger = Logger.getLogger(RequestMessageV2.class);
    ...
    public static RequestMessageV2 toObject(String jsonStr) {
        RequestMessageV2 result = null;
        try {
            mapper.getFactory().configure(JsonGenerator.Feature.ESCAPE_NON_ASCII, false);
            result = mapper.readValue(jsonStr, RequestMessageV2.class);
        } catch (IOException e) {
            logger.error(Tool.getLogMessage(e));
        }
        return result;
