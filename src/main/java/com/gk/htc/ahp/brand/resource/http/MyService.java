...
import com.gk.htc.sendMT.service.NewProcess;
...
public class MyService {
    ...
    static final Logger logger = Logger.getLogger(MyService.class);
    @Context
    HttpServletRequest request;

    @POST
    @Consumes(MediaType.APPLICATION_JSON)
//    public Response doPostJson(RequestMessage reqData) {
    public Response doPostJson(String data) {
        ...
        try {
            ...
            if (reqData == null) {
                resp.setCode(NewProcess.CODE.INVALID_JSON_DATA.val);
                resp.setMessage(NewProcess.CODE.INVALID_JSON_DATA.mess);
            ...
            //-- Call
            NewProcess process = new NewProcess(reqData);
            resp = process.process();
            return Response.status(HttpStatus.ORDINAL_200_OK).entity(resp.toJsonStr()).build();
        } catch (Exception e) {
            logger.error(Tool.getLogMessage(e));
            resp.setCode(NewProcess.CODE.EXCEPTION.val);
            resp.setMessage(NewProcess.CODE.EXCEPTION.mess);
