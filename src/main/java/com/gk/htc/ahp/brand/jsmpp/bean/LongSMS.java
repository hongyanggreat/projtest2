...
import com.gk.htc.ahp.brand.jsmpp.util.HexUtil;
...
public class LongSMS {
    ...
    public static byte[][] splitMessage8Bit(byte[] aMessage) {
        ...
        for (int i = 0; i < segmentNum; i++) {
            ...
            Tool.debug("Length of data = " + lengthOfData);

            segments[i] = new byte[7 + lengthOfData];
            Tool.debug("segments[" + i + "].length = "
                    + segments[i].length);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < splittedMsg.length; i++) {
            Tool.debug(splittedMsg[i].length + "");
            Tool.debug(new String(splittedMsg[i]));
            // Tool.debug("sar_msg_refnum tag: " +
            // OctetUtil.bytesToShort(splittedMsg[i], 0));
            Tool.debug("sar_msg_refnum tag: "
                    + HexUtil.convertBytesToHexString(splittedMsg[i], 0, 2));
            Tool.debug("sar_msg_refnum length: "
                    + OctetUtil.bytesToShort(splittedMsg[i], 2));
            Tool.debug("sar_msg_refnum value: "
