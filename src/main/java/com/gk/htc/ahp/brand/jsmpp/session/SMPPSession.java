...
import com.gk.htc.ahp.brand.jsmpp.DefaultPDUReader;
import com.gk.htc.ahp.brand.jsmpp.DefaultPDUSender;
import com.gk.htc.ahp.brand.jsmpp.InvalidCommandLengthException;
import com.gk.htc.ahp.brand.jsmpp.InvalidResponseException;
import com.gk.htc.ahp.brand.jsmpp.PDUException;
import com.gk.htc.ahp.brand.jsmpp.PDUReader;
...
import com.gk.htc.ahp.brand.jsmpp.SynchronizedPDUSender;
...
import com.gk.htc.ahp.brand.jsmpp.session.connection.socket.SocketConnectionFactory;
import com.gk.htc.ahp.brand.jsmpp.util.DefaultComposer;
...
public class SMPPSession extends AbstractSession implements ClientSession {
    ...
    public String connectAndBind(String host, int port, BindParameter bindParam, long timeout) throws IOException {
        ...
        this.host = host;
        this.port = port;
        this.addRessRange = bindParam.getAddressRange();
        //--
    ...
    private class PDUReaderWorker extends Thread {
        ...
        private final ExecutorService executorService = Executors.newFixedThreadPool(getPduProcessorDegree());

        private final Runnable onIOExceptionTask = new Runnable() {
            @Override
            public void run() {
                close();
            }
	    ...
	    @Override
        public void run() {
            MyLog.debug("Starting PDUReaderWorker with processor degree:{" + getPduProcessorDegree() + "} ...");
            while (isReadPdu()) {
                readPDU();
            }
            close();
            executorService.shutdown();
            MyLog.debug("PDUReaderWorker stop");
        }
...
//                    MyLog.debug("EnquireLinkSender runing");
                ...
                if (distance > 30 * 1000) {
                    Tool.debug(DateProc.createTimestamp() + "--> EnquireLinkSender live: [" + host + ":" + port + ":" + addRessRange + "]");
                    working.doneCycle();
                }
            }
            MyLog.debug("EnquireLinkSender stop");
        }
