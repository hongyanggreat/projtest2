...
public enum StringParameter {
    ...
    /**
     * Invalid address range assume should return the error of invalid source
     * address.
     */
    ...
    /**
     * When validating final date is failed, then we should throw error with
     * status STAT_ESME_RINVDFTMSGID, means that predefined message are not
     * exist.
     */
    ...
    /**
     * ESME_ADDR has error code OK because there is not response to
     * alert_notification (ESME_ADDR only use on alert_notification), so there
     * is no error code to return.
     */
